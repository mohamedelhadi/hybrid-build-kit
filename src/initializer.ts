...
async function prepareCordovaConfig(env: string) {
  ...
  return new Promise((resolve, reject) => {
    ...
    $('widget').attr(
      'android-versionCode',
      details.versionDetails.androidVersionCode
    );
...
async function prepareIndex(env: string, platform: string) {
  ...
  const cordovaScript =
    env === environments.browser || platform === 'pwa' ? '' : 'cordova.js';
  ...
  return new Promise((resolve, reject) => {
    fs.writeFile(
      ...
      html,
      callback(
        'Done preparing index.html',
        'Could not save index.html!',
        resolve,
        reject
      )
    );
    ...
    fs.readFile(whitelistPath, 'utf8', (err, data) => {
      ...
      for (const key in whitelist) {
        if (key === env || key === 'default') {
          ...
          directives.styleSrc = directives.styleSrc.concat(
            configs.styleSrc || []
          );
